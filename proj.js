...
function renderVirtualNotebook() {
  var subjectsWithTeachers = [
    { name: 'English', teacher: 'Ingle Ish', ay: '2025-2026', section: '9-Ruby' },
    { name: 'Japanese', teacher: 'Jhapah Ness', ay: '2025-2026', section: '9-Ruby' },
    { name: 'Math', teacher: 'Matthew Matics', ay: '2025-2026', section: '9-Ruby' },
    { name: 'Science', teacher: 'Sy Yens', ay: '2025-2026', section: '9-Ruby' },
    { name: 'History', teacher: 'Hayes Tory', ay: '2025-2026', section: '9-Ruby' }
  ];

  var cards = '';
  for (var s = 0; s < subjectsWithTeachers.length; s++) {
    var sub = subjectsWithTeachers[s];
    var saved = loadNote(sub.name);
    var isActive = (sub.name === currentSubject) ? 'active-subject-card' : '';
    
    cards +=
      '<div class="subject-card ' + isActive + '" onclick="selectSubject(\'' + sub.name + '\')">' +
        '<div class="subject-card-header">' +
          '<h3 class="subject-name">' + sub.name + '</h3>' +
          '<div class="subject-year">' + sub.ay + '</div>' +
        '</div>' +
        '<div class="subject-card-body">' +
          '<p class="teacher-name">' + sub.teacher + '</p>' +
          '<p class="section-name">' + sub.section + '</p>' +
        '</div>' +
        '<div class="subject-card-footer">' +
          '<span class="note-indicator">' + (saved.trim() ? 'Has notes!' : 'Empty!') + '</span>' +
          '<button class="write-note-btn" onclick="event.stopPropagation(); openNoteEditor(\'' + sub.name + '\')">Write Notes</button>' +
        '</div>' +
      '</div>';
  }
  ...
  return '<div class="container">' +
    '<div class="notebook-dashboard">' +
      '<div class="notebook-header">' +
        '<h1 class="notebook-title">Virtual Notebook</h1>' +
        '<p class="notebook-subtitle">Select a subject to view or write your notes</p>' +
      '</div>' +
      '<div class="subjects-grid">' + cards + '</div>' +
    '</div>' +
    '<div id="noteEditorModal" class="note-editor-modal" style="display:none;">' +
      '<div class="note-editor-content">' +
        '<div class="note-editor-header">' +
          '<h3 id="editorSubjectTitle">Subject Notes</h3>' +
          '<button class="close-editor" onclick="closeNoteEditor()">&times;</button>' +
        '</div>' +
        '<textarea id="noteEditorTextarea" rows="12" placeholder="Write your notes here..."></textarea>' +
        '<div class="note-editor-footer">' +
          '<button class="save-note-btn" onclick="saveCurrentNote()">Save Notes</button>' +
          '<button class="cancel-note-btn" onclick="closeNoteEditor()">Cancel</button>' +
        '</div>' +
        '<div id="editorSaveMsg" class="save-msg"></div>' +
      '</div>' +
    '</div>' +
  '</div>';
}
...
function selectSubject(subject) {
  ...
  for (var i = 0; i < cards.length; i++) {
    cards[i].classList.remove('active-subject-card');
  }
  var activeCard = document.querySelector('.subject-card h3');
  if (activeCard && activeCard.innerText === subject) {
    activeCard.closest('.subject-card').classList.add('active-subject-card');
  }
}

function openNoteEditor(subject) {
  currentSubject = subject;
  var savedNotes = loadNote(subject);
  document.getElementById('editorSubjectTitle').innerText = subject + ' Notes';
  document.getElementById('noteEditorTextarea').value = savedNotes;
  document.getElementById('noteEditorModal').style.display = 'flex';
}

function closeNoteEditor() {
  document.getElementById('noteEditorModal').style.display = 'none';
  document.getElementById('editorSaveMsg').innerText = '';
}

function saveCurrentNote() {
  var notes = document.getElementById('noteEditorTextarea').value;
  saveNote(currentSubject, notes);
  var msgDiv = document.getElementById('editorSaveMsg');
  msgDiv.innerText = currentSubject + ' notes saved!';
  msgDiv.style.color = '#1e7a45';
  setTimeout(function() {
    msgDiv.innerText = '';
  }, 2000);
  ...
  var cards = document.querySelectorAll('.subject-card');
  for (var i = 0; i < cards.length; i++) {
    var nameEl = cards[i].querySelector('.subject-name');
    if (nameEl && nameEl.innerText === currentSubject) {
      var indicator = cards[i].querySelector('.note-indicator');
      if (indicator) {
        indicator.innerText = notes.trim() ? '📝 Has notes' : '📭 Empty';
      }
      break;
    }
  }
